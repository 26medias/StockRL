...
class TradingGameEnv(gym.Env):
  """
  Custom Environment
  Teaches the basics rules of the game:
  One position at a time. Can't sell if you don't have a position open. Can't buy again if you already have a position.
  """
  ...
  def step(self, action):
    ...
    if action == self.BUY:
      ...
      # Wait
      if self.game.position==False:
        reward = 0
      else:
        reward = (self.game.positionGains()/10 + self.game.getChange(5)) * self.dec(self.game.positionAge(), maxHold, 1)
