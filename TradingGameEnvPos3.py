...
class TradingGameEnv(gym.Env):
  """
  Custom Environment
  Teaches to drop losing positions and hold winning positions
  """
  ...
  def step(self, action):
    ...
    if action == self.BUY:
      ...
      if sellResponse == False:
        ...
        if sellResponse>0:
          reward = sellResponse*5
        else:
          reward = abs(sellResponse)/3
    elif action == self.WAIT:
      # Wait
      if self.game.position>0:
        posGains    = self.game.positionGains()
        change2     = self.game.getChange(2)
        change5     = self.game.getChange(5)
        change10    = self.game.getChange(10)
        posAge      = self.game.positionAge()
        if posAge>=10:
          reward = (((change2*0.2) + (change5*0.3) + (change10*0.5) + posGains)/2) / 5
        elif posAge>=5:
          reward = ((change2*0.5) + (posGains*0.5)) / 5
        else:
          reward = posGains/5
      else:
        reward = 0
