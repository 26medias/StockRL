...
class TradingGameEnv(gym.Env):
  """
  Custom Environment
  Teach to detect trading opportunities before buying
  """
  ...
  def decList(self, vals):
    a = np.arange(len(vals),0,-1)
    a = a/np.sum(a)
    return np.mean(vals * a)
  ...
  def step(self, action):
    ...
    futureScore = self.decList(self.game.getFutureStepData())
    ...
    if action == self.BUY:
      ...
      if sellResponse == False:
        ...
        if sellResponse>0:
          reward = -futureScore*5
        else:
          reward = abs(sellResponse)/3
    elif action == self.WAIT:
      # Wait
      if self.game.position>0:
        reward = futureScore/2
      else:
        reward = 0
    
    #reward = futureScore
    
    #print(self.game.cursor, futureScore, "\t", list(self.game.getFutureStepData()), "\t", list(self.game.getStepData()[-6:]))
